...
# ダメージ適用
    scoreboard players operation #mhdp_diablos_health AsaMatrix += #diablos_damage AsaMatrix
    scoreboard players operation #mhdp_diablos_health_half AsaMatrix += #diablos_damage AsaMatrix
# ボスバー設定
...
# 体力半分時処理
    execute as @e[type=armor_stand,tag=DiablosRoot] if entity @s[tag=!HpHalf] if score #mhdp_diablos_health_half AsaMatrix matches ..0 run function mhdp_core:phase/3_quest/check/hp_half
