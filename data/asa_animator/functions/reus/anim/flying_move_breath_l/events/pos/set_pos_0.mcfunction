...
# markerセット
summon marker ^-10 ^ ^32 {Tags:["ReusMovePos"]}
execute unless entity @e[tag=ReusAttackTarget,distance=..30] rotated ~ 0 positioned as @e[tag=ReusAttackTarget] positioned ^-5 ^3 ^5 run tp @e[type=marker,tag=ReusMovePos] ~ ~ ~
...
scoreboard players set #reus_height_target AsaMatrix 36
scoreboard players operation #reus_mine_x AsaMatrix /= #reus_height_target AsaMatrix
scoreboard players operation #reus_mine_y AsaMatrix /= #reus_height_target AsaMatrix
scoreboard players operation #reus_mine_z AsaMatrix /= #reus_height_target AsaMatrix
