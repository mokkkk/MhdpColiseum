...
# 近距離，確立でBJB
    execute unless entity @s[tag=AnmHardFjb] if predicate asa_animator:random/070 if entity @e[distance=..13,tag=ReusAttackTarget] run tag @s add AnmBjb

# その他，通常飛行
    execute unless entity @s[tag=AnmHardFjb] unless entity @s[tag=AnmBjb] run tag @s add AnmFly