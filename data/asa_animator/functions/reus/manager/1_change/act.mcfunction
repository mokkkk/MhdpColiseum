...
# 遷移
    ...
    # 飛行（）
        execute if predicate asa_animator:reus/stay unless score #mhdp_reus_actcount_phase AsaMatrix matches 5.. if entity @s[tag=IsFlying] run function asa_animator:reus/manager/1_change/4_flying/
    # 飛行→地上
        execute if predicate asa_animator:reus/stay if score #mhdp_reus_actcount_phase AsaMatrix matches 5.. if entity @s[tag=IsFlying] run function asa_animator:reus/manager/1_change/5_end_fly/
