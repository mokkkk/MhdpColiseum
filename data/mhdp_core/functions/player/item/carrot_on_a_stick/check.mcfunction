#> mhdp_core:player/item/carrot_on_a_stick/check
#
# アイテム用メイン処理 いろいろ キャンセル検知
...
# 入れ替えを検知した場合，使用を中断
    ...
    execute if entity @s[scores={MhdpTUsingCoasItem=3..}] if score #mhdp_temp_success MhdpCore matches 1.. run say 使用中断
