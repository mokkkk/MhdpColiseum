#> mhdp_core:player/damage/knockback/wind/0
#
# プレイヤーノックバック開始処理

# スキル確認
    execute if entity @s[tag=SklWindproofLv1] run tag @s add NotKnockback
...
# アーマー確認
    execute if entity @s[tag=PlyArmorSuper] run tag @s add NotKnockback
...
# スキルで無効化されていない場合，風圧処理実行
    execute unless entity @s[tag=NotKnockback] run function mhdp_core:player/damage/knockback/wind/0_
