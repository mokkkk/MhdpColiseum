#> mhdp_weapons:weapon/bow/5_charge_step/end
#
# 弓：チャージステップ終了
...
# 溜め段階を１段階上げる
    ...
    execute if entity @s[tag=WpnBowCharge3,tag=!SklBowChargePlus] run scoreboard players set @s MhdpWeaponCharge 600
    execute if entity @s[tag=WpnBowCharge3,tag=SklBowChargePlus] run scoreboard players set @s MhdpWeaponCharge 800
...
# 遷移
    # 右クリック中の場合，溜めに移行
        execute if entity @s[tag=PlyUsingEyeCurrent] run function mhdp_weapons:weapon/bow/2_charge/start_combo_cstep
    # 右クリック中じゃない場合，射撃に移行
        execute if entity @s[tag=!PlyUsingEyeCurrent] at @s run function mhdp_weapons:weapon/bow/3_shot/start
