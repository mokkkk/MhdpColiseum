...
# 引き絞りモーション
    execute if entity @s[scores={MhdpWeaponTimer=..9}] run scoreboard players add @s MhdpWeaponTimer 1
...
# 遷移
    # 集中力が0以下になったとき，強制的に射撃に移行
        execute if entity @s[scores={MhdpWeaponExt=..0}] run function mhdp_weapons:weapon/bow/0_charge/change_shot
    # 右クリックを解除したとき，射撃に移行
        execute if entity @s[tag=PlyUsingEyeEnd,scores={MhdpWeaponExt=1..}] run function mhdp_weapons:weapon/bow/0_charge/change_shot
    # 2回スニークしたとき，チャージステップに移行
        execute if entity @s[tag=PlySneakDouble,scores={MhdpWeaponExt=1..}] run function mhdp_weapons:weapon/bow/0_charge/change_charge_step
