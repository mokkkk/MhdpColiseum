#> mhdp_weapons:weapon/short_sword/14_just_3/animation_0
#
# 片手剣：ジャストラッシュ１ アイテムアニメーション
...
# 武器ステータス変更
    ...
    # Cmd計算
        ...
        scoreboard players add #mhdp_temp_cmd MhdpCore 48
    ...
    # Cmd計算
        scoreboard players remove #mhdp_temp_cmd MhdpCore 34
        execute store result storage mhdp_core:temp Temp.CustomModelData int 1 run scoreboard players get #mhdp_temp_cmd MhdpCore
    # Attribute削除
        data modify storage mhdp_core:temp Temp.AttributeModifiers set value []
    # ダミー用データ
