...
# 武器アニメーション
    ...
    execute if entity @s[scores={MhdpWeaponTimer=3}] run function mhdp_weapons:weapon/great_sword/3_charge_attack/animation_0
    execute if entity @s[scores={MhdpWeaponTimer=15}] run function mhdp_weapons:weapon/great_sword/3_charge_attack/animation_1

# 攻撃
    execute if entity @s[scores={MhdpWeaponTimer=3}] run function mhdp_weapons:weapon/great_sword/8_edge/attack

# 遷移
    # ジャンプ時，ステップ回避に移行
        execute if entity @s[scores={MhdpJump=1..,MhdpWeaponTimer=16..44}] run function mhdp_core:player/sneak/avoid/start_
    # 攻撃ヒット時，溜めに移行
        execute if entity @s[tag=AttackHit,scores={MhdpWeaponTimer=4..5}] run tp @s @s
        execute if entity @s[tag=AttackHit,scores={MhdpWeaponTimer=6}] run function mhdp_weapons:weapon/great_sword/8_edge/change_charge
